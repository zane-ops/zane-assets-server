...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      let maybeSecondRequest = request.clone(), routeToUserWorker = async ({
        asset
      }) => {
        ...
        return eyeballConfig.limitedAssetsOnly ? (analytics.setData({ userWorkerFreeTierLimiting: !0 }), new Response(renderLimitedResponse(maybeSecondRequest), {
          status: 429,
          headers: {
          ...
          let shouldBlockNonImageResponse = !1;
          if (url.pathname.endsWith("/_next/image")) {
            let queryURLParam = url.searchParams.get("url");
            queryURLParam && !queryURLParam.startsWith("/") && (maybeSecondRequest.method !== "GET" || maybeSecondRequest.headers.get("sec-fetch-dest") !== "image") && (shouldBlockNonImageResponse = !0, analytics.setData({ abuseMitigationURLHost: queryURLParam }));
          }
          if (analytics.setData({
            timeToDispatch: performance.now() - startTimeMs
          }), shouldBlockNonImageResponse) {
            let resp = await env.USER_WORKER.fetch(maybeSecondRequest), contentType = resp.headers.get("content-type") || "";
            return !(contentType.startsWith("image/") || // Matches "text/plain", "text/plain;charset=UTF-8"
            contentType.split(";")[0] === "text/plain") && resp.status !== 304 ? (analytics.setData({ abuseMitigationBlocked: !0 }), new Response("Blocked", { status: 403 })) : resp;
          }
